...
В репозитории **5 готовых bat-скриптов** для быстрой конвертации видео и изображений через NVIDIA GPU.

> ⚠️ **Требование:** видеокарта NVIDIA серии RTX 30** или 40**. Серия RTX 50** (Blackwell) **не поддерживает** аппаратное кодирование H.264 (`h264_nvenc`) — для неё используйте `hevc_nvenc` или `av1_nvenc`. Все скрипты ниже используют `hevc_nvenc`, поэтому на RTX 50** работают без изменений.

| Скрипт | Описание |
| ------ | -------- |
| `⚡conv-nvenc-SendTo.bat` | Конвертация одного видео через NVENC (H.265). Настройка FPS, разрешения, GOP. Файл сохраняется рядом с оригиналом |
| `⚡conv-img-SendTo.bat` | Пакетная конвертация изображений (JPG, PNG, WebP, BMP, TIFF, AVIF, HEIC). Выбор формата, качества, масштаба. Отчёт в `converted-img` |
| `⚡conv-all-nvenc-orig-done-smart-stat.bat` | Пакетное сжатие всех MP4 в текущей папке. Авто-ограничение FPS >30 до 30, умный пропуск если файл не уменьшился. Оригиналы в `done`, сжатые в `converted`. Отчёт `conversion_report.txt` |
| `⚡conv-all-nvenc-orig-done-smart-stat-SendTo.bat` | То же, что выше, но через меню «Отправить» (правый клик → SendTo) |
| `✂️cut-&-nvenc-SendTo.bat` | Обрезка видео по времени + конвертация через NVENC. Ввод начала/конца в формате `00:00:00`. Файл `_cut.mp4` рядом с оригиналом |

## Содержание

1. [Готовые bat-скрипты](#готовые-bat-скрипты)
2. [Что такое FFmpeg](#что-такое-ffmpeg)
3. [Установка](#установка)
   - [Windows](#windows)
   - [macOS](#macos)
   - [Linux](#linux)
4. [Основные концепции](#основные-концепции)
5. [Синтаксис команд](#синтаксис-команд)
6. [Глобальные параметры](#глобальные-параметры)
7. [Кодеки и форматы](#кодеки-и-форматы)
8. [Конвертация видео](#конвертация-видео)
9. [Конвертация аудио](#конвертация-аудио)
10. [Работа с изображениями](#работа-с-изображениями)
11. [Фильтры и обработка видео](#фильтры-и-обработка-видео)
12. [Обрезка и монтаж](#обрезка-и-монтаж)
13. [Водяные знаки и субтитры](#водяные-знаки-и-субтитры)
14. [Сжатие и качество](#сжатие-и-качество)
15. [Аппаратное ускорение](#аппаратное-ускорение)
16. [Запись экрана и стриминг](#запись-экрана-и-стриминг)
17. [Пакетная обработка](#пакетная-обработка)
18. [Диагностика и анализ файлов](#диагностика-и-анализ-файлов)
19. [Частые ошибки и решения](#частые-ошибки-и-решения)
20. [Лайфхак для Windows: запуск через правую кнопку мыши](#лайфхак-для-windows-запуск-через-правую-кнопку-мыши)
