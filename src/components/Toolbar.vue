    ...
    <div class="mt-4">
      <b-button-group>
        <b-button
          ...
          @click="encode">{{ encoding ? 'Кодирование...' : 'Кодировать' }}
        </b-button>
        ...
        <b-dropdown
          ...
          <b-dropdown-item @click="toggleJSON">
            {{ value.showJSON ? 'Скрыть' : 'Показать' }} JSON
          </b-dropdown-item>
        </b-dropdown>
      </b-button-group>
      ...
      <b-button-group class="float-right">
        ...
        >
          ...
          <b-dropdown-item @click="deleteAllPresetsPrompt">
            Удалить все сохранённые пресеты
          </b-dropdown-item>
        </b-dropdown>
      </b-button-group>
    </div>
    ...
    <b-overlay :show="showDeletePrompt" no-wrap @shown="onShown">
      <template v-slot:overlay>
        <div ref="dialog" class="text-center p-3">
          <p>Вы уверены, что хотите удалить все сохранённые пресеты?</p>
          <div>
            <b-button class="mr-3" @click="onCancel">Отмена</b-button>
            <b-button variant="outline-danger" @click="onOK">Удалить все сохранённые пресеты</b-button>
          </div>
        </div>
      </template>
    </b-overlay>
