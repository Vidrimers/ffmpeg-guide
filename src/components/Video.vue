  ...
  <div>
    ...
    <b-form-row>
      <b-col v-if="value.size === 'custom'">
        <b-form-group label="Ширина:" label-for="width">
          <b-form-input
      ...
      <b-col v-if="value.size === 'custom'">
        <b-form-group label="Высота:" label-for="height">
          <b-form-input
    ...
    <b-form-group
      ...
      label="Параметры кодека:"
      label-for="codec-options">
      <b-form-textarea
        id="codec-options"
        v-bind:value="value.codec_options"
        @input="update('codec_options', $event)"
        :placeholder="`Укажите необязательные параметры -${value.codec}- для переопределения опций кодировщика.`"
        rows="1"
        max-rows="6"
      />
    </b-form-group>
...
export default {
  ...
  data() {
    return {
      ...
      bitrateItems: [
        { name: 'Битрейт', value: 'bitrate' },
        { name: 'Мин. битрейт', value: 'minrate' },
        { name: 'Макс. битрейт', value: 'maxrate' },
        { name: 'Размер буфера', value: 'bufsize' },
        { name: 'Размер GOP', value: 'gopsize', supported: ['x264', 'vp9'] },
      ],
      videoEditItems: [
